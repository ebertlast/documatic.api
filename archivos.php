...
//region Adaptando los headers permitidos en las peticiones
...
//region C O N V E N C I O N E S
// get convencion
...
$app->get("/archivo-download/{archivoid}", function ($request, $response, $args) use ($db, $app) {
    ...
    if (!$aprobado && !($convencionid === 'F' || $convencionid === 'C')) {
    ...
    $firmas = array();
    if ($query){//} && !($convencionid === 'F' || $convencionid === 'C')) {
        while ($fila = $query->fetch(PDO::FETCH_ASSOC)) {
    ...
    // return $response
    //         ->withHeader('Content-type', 'application/json')
    //         ->withJson(array('error' => $convencionid))
    //         ;
...
$app->post("/archivo/{archivoid}", function ($request, $response, $args) use ($db, $app) {
    ...
    if ($ext === 'pdf') {
        ...
        if (!$aprobado) {
            // return $response
            // ->withHeader('Content-type', 'application/json')
            // ->withJson(array('error' => json_encode($pdfDestino)))
            // ;
            ...
            $sql = "UPDATE `{$tabla}` SET `archivoid`='{$modelo['archivoid']}',`nombre`='{$modelo['nombre']}',`gestionid`='{$modelo['gestionid']}',`convencionid`='{$modelo['convencionid']}',`archivoidaux`='{$modelo['archivoidaux']}',`denominacion`='{$modelo['denominacion']}',`observaciones`='{$modelo['observaciones']}',`usuario`='{$modelo['usuario']}',`fecha`=NOW(),`fechaexp`=DATE_ADD(NOW(), INTERVAL 5 YEAR) WHERE `archivoid`='{$args['archivoid']}'";
            // return $response
            //         ->withHeader('Content-type', 'application/json')
            //         ->withJson(array('error' => json_encode($sql)))
            //         ;
            ...
            if ($update) {
                $firmas = array();
                firmar($firmas, $pdfOrigen, $pdfDestino, true);
                $data = array('filename' => $pdfDestino, 'success' => true);
                return $response
                    ->withHeader('Content-type', 'application/json')
                    ->withJson(array("data" => $data, "token" => newToken($dataUser)));
            }
        }
...
//region  L I S T A D O  M A E S T R O
$app->get("/listadomaestro", function ($request, $response, $args) use ($db, $app) {
    ...
    $data = array('success' => true, 'filename' => $pdfDestino);
    return $response
        ->withHeader('Content-type', 'application/json')
        ->withJson(array("status" => "success", "data" => $data, "token" => newToken($dataUser)))
    ;
...
//region A P R O B A C I O N E S
